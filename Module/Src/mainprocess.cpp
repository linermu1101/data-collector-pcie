    ...
    , ui(new Ui::MainProcess)
{
   ...
   //  if(!busList.contains(1)){
   //      ui->Filpcard1->hide();
   //  }
   //  if(!busList.contains(2)){
   //       ui->Filpcard2->hide();
   //  }
   //  if(!busList.contains(3)){
   //      ui->Filpcard3->hide();
   // }
   //  if(!busList.contains(4)){
   //      ui->Filpcard4->hide();
   // }
   //  if(!busList.contains(5)){
   //      ui->Filpcard5->hide();
   // }
   //  if(!busList.contains(6)){
   //      ui->Filpcard6->hide();
   // }
   //  if(!busList.contains(7)){
   //      ui->Filpcard7->hide();
   // }
    ...
    // {
    //     PcieFunction *pDev = new PcieFunction(i);
    //     QThread *pThread = new QThread(this);
    //     hDevList.append(pDev);
    //     threadList.append(pThread);
    //     pDev->moveToThread(pThread);
    //     pThread->start();

    //     connect(this,SIGNAL(openDev(QString,int)),pDev,SLOT(openPcieDev(QString,int)));
    //     connect(this,SIGNAL(closeDev()),pDev,SLOT(closePcieDev()));
    //     connect(this,SIGNAL(readData(PlanarGraph *,SettingPara)),pDev,SLOT(c2h_transfer(PlanarGraph *,SettingPara)));
    //     connect(this,SIGNAL(readCfg(UINT32,UINT32,unsigned char *,int)),pDev,SLOT(user_read(UINT32,UINT32,unsigned char *,int)));
    //     connect(this,SIGNAL(writeCfg(UINT32,UINT32,unsigned char *,int)),pDev,SLOT(user_write(UINT32,UINT32,unsigned char *,int)));
    //     connect(pDev,SIGNAL(replotChart(int )),this,SLOT(replotChart(int)));
    //     connect(pDev,SIGNAL(startHardCollection()),this,SLOT(revHardTrigStart()));
    //     connect(this,SIGNAL(readIrq(int,int)),pDev,SLOT(user_read_IRQ(int,int)));
    //     connect(pDev,SIGNAL(timeoutIRQ()),this,SLOT(hardTrigTimeout()));
    //     connect(pDev,SIGNAL(ddrEnd(int)),this,SLOT(ddrProcess(int)));
    //     //connect(pDev, &PcieFunction::realdataLoaded, this, &MainProcess::handleRealProgress, Qt::UniqueConnection);
    //     connect(pDev, &PcieFunction::report, this, [this](QString msg) {
    //             this->Log(msg);
    //         }, Qt::UniqueConnection);

    //      // SliderRuler *ruler = new SliderRuler;
    //      // ruler->setBgColor(Qt::transparent,Qt::transparent);
    //      // ruler->setRange(0,5);
    //      // ruler->setPrecision(1);
    //      // ruler->setStep(0.5,0.1);

    //      // for(int i=0; i<6; i++)
    //      // {
    //      //     if(i<3)
    //      //        ruler->addSlider(Qt::red);
    //      //     else
    //      //        ruler->addSlider(Qt::green);
    //      // }
    //      // rulerList.append(ruler);
    //     // PlanarGraph *m_plot = new PlanarGraph(QString::number(busList[i]));
    //     // m_plot->plot_2D_Init(Qt::white,5,colorList);
    //     // plotList.append(m_plot);
    //     // connect(m_plot,SIGNAL(windowFullorRestoreSize(int,bool)),this,SLOT(maximizingSingleWindow(int,bool)));
    //     // emit openDev(pathList[i],i);
    //     // ui->hardtrigIndex->addItem("卡"+QString::number(busList[i]));
    // }
    ...
    //  if(!busLists.contains(1)){
    //      ui->Filpcard1->hide();
    //  }
    //  if(!busLists.contains(2)){
    //       ui->Filpcard2->hide();
    //  }
    //  if(!busLists.contains(3)){
    //      ui->Filpcard3->hide();
    // }
    //  if(!busLists.contains(4)){
    //      ui->Filpcard4->hide();
    // }
    //  if(!busLists.contains(5)){
    //      ui->Filpcard5->hide();
    // }
    //  if(!busLists.contains(6)){
    //      ui->Filpcard6->hide();
    // }
    //  if(!busLists.contains(7)){
    //      ui->Filpcard7->hide();
    // }
    ...
    for(int k = 0; k < cardNumbers; k ++)
    {
        ...
        //QString pciePathItem = ""; // test code
        qDebug() << " PCI path(byCfg):" << pciePathItem;

        QString info = QString("识别到第%1张卡 %2").arg(k+1).arg(pciePathItem);
        Log(info);

        QThread *threadDataLoader = new QThread();
        QThread *threadRealDataLoader = new QThread();
        //QThread *threadPcie = new QThread();
        QThread *threadReal = new QThread();


        //PcieProtocol *pcie = new PcieProtocol(nullptr,  pciePathItem, k);
        DataLoader *dataLoader = new DataLoader(nullptr, k);
        RealDataLoader *realdataLoader = new RealDataLoader(nullptr, k);
        PcieProtocol *pcieReal = new PcieProtocol(nullptr,  pciePathItem, k);//采用map形式存储卡号（int，0开始）和对应地址pciePathItem在PcieProtocol构造函数中初始化
...
//        rulerList.append(ruler);
        // 图表列表
        PlanarGraph *m_plot = new PlanarGraph(QString::number(k));//new PlanarGraph(QString::number(busLists[k]));
        ...
        // ui->hardtrigIndex->addItem("卡"+QString::number(busLists[k]));
    }

    for(int c = 1; c <= 50; c++)
    {
        ui->hardtrigIndex->addItem(QString::number(c));
    }
    connect(this,&MainProcess::hardCollectionTrigger,this,&MainProcess::startCollection);
    ...
    connect(referenceTimer,&QTimer::timeout,this,[=](){
        for(int i=0 ;i<cardNumbers; i++){
           ...
           // hDevList[i]->user_read(Frequency_Reference_Addr,4,buf,i);
//            if(modeIndex){
//                float value = (float)(buf[2]+i)*10/256;
//                rulerList[i]->setValue(0,value);
//                float value2 = (float)(buf[1]+i)*10/256;
//                rulerList[i]->setValue(1,value2);
//                float value3 = (float)(buf[0]+i)*10/256;
//                rulerList[i]->setValue(2,value3);
//            }else{
//                float value = (float)(buf[1])*10/256;
//                rulerList[i]->setValue(0,value);
//            }
        }
    });
    ...
    connect(measureTimer,&QTimer::timeout,this,[=](){
        for(int i=0 ;i<cardNumbers; i++){
            unsigned char *buf;
            buf = (unsigned char*)malloc(4);
            //hDevList[i]->user_read(Measuring_Frequency_Addr,4,buf,i);
//           if(modeIndex){
//                float value = (float)(buf[2]+i)*10/256;
//                rulerList[i]->setValue(0,value);
//                float value2 = (float)(buf[1]+i)*10/256;
...
//            }
    ...
    // referenceTimer->start(10);
    // measureTimer->start(10);
    ...
    //新增：示波器功能
    // 初始化自动采集选择框 - 在适当位置添加（建议在initToolBarCtl()调用之后）
    ...
    readIniFile();
    setTrigMainCard();
    this->setWindowTitle("红外激光干涉仪数据采集系统  V250928");
    ...
    prePareDir();
//    qDebug()<<"txtdensity_ratio_b:  "<< ui->txtdensity_ratio_b->text().toInt();
//    qDebug()<<"txtchange_rt:  "<< ui->txtchange_rt->text().toInt();
//    qDebug()<<"txtsymb_on:  "<< ui->txtsymb_on->text().toInt();
//    qDebug()<<"txtsymbol:  "<< ui->txtsymbol->text().toInt();
//    qDebug()<<"txtsym_time:  "<< ui->txtsym_time->text().toInt();
//    qDebug()<<"txtsingal_mode:  "<< ui->txtsingal_mode->text().toInt();
    //showSignalValueWindow();
}
...
void MainProcess::initToolBarCtl()
{
    ...
    // 实时数据显示已直接添加在采集结束后面，DDR显示有bug，关闭
//    ui->toolBar->addWidget(btnRealPlotView);
//    ui->toolBar->addWidget(btnHistoryPlotView);
...
void MainProcess::initReflectFileInfWidget()
{
    ...
    ui->tableWidget->setColumnCount(6); // 增加一列用于卡存储开关
   ...
   for(int i=0; i<cardNumbers; i++)
   {
       ...
       for(int j=0; j<5; j++)
       {
           ...
           idEdit->setText(QString::number(j+1)); // 默认ID值
       ...
       storageBtn->setObjectName(QString("cardStorageBtn_%1").arg(i+1)); // 设置对象名便于查找
       storageBtn->setStyleSheet("QPushButton{min-width:20px;min-height:20px;border-radius:10px;"
       ...
       storageBtn->setChecked(true); // 默认选中
   ...
   ui->tableWidget_2->verticalHeader()->setMinimumSectionSize(60);
...
void MainProcess::readIniFile()
{
    ...
    // 根据卡存储状态初始化或删除图表
    for (int i = 0; i < cardNumbers; i++) {
        if (cardStorageStates[i]) {
            // 卡存储开关打开，需要创建图表
            if (i >= plotList.size() || plotList[i] == nullptr) {
                // 创建新的图表
                ...
                pConfig.beginGroup("CHANNEL_PARA");
                QList<QColor> colorList;
                QString colorStr = pConfig.value("Card" + QString::number(i + 1) + "_chart_colors").toString();
                if (!colorStr.isEmpty()) {
                    QStringList colorNames = colorStr.split(",");
                    for (const QString &name : colorNames) {
                        colorList.append(QColor(name));
                    }
                } else {
                    // 默认颜色列表
                    colorList = {Qt::red, Qt::blue, Qt::green, Qt::yellow, Qt::magenta};
                }

                int lineWidth = pConfig.value("Card" + QString::number(i + 1) + "_line_width", 3).toInt();
                QColor bgColor = QColor(pConfig.value("Card" + QString::number(i + 1) + "_background_color", "#ffffff").toString());
                pConfig.endGroup();

                // 初始化图表
                m_plot->plot_2D_Init(bgColor, lineWidth, colorList);
...
//                rulerList[i] = nullptr;
    ...
    // 读取窗口状态信息
    pConfig.beginGroup("WINDOW_STATE");
    for (int i = 0; i < plotList.size(); i++) {
        if (plotList[i] != nullptr) {
            bool maximized = pConfig.value("Window_" + QString::number(i) + "_maximized", false).toBool();
            QByteArray geometry = pConfig.value("Window_" + QString::number(i) + "_geometry").toByteArray();

            if (!geometry.isEmpty()) {
                plotList[i]->restoreGeometry(geometry);
            }
            if (maximized) {
                plotList[i]->showMaximized();
            }
        }
    }
    pConfig.endGroup();
...
void MainProcess::saveIniFile()
{
    ...
    pConfig.setValue("Feekback_Way", str);
    ...
    for (int i = 0; i < cardNumbers; i++) {
        ...
        // 只保存存在的图表配置
        if (i < plotList.size() && plotList[i] != nullptr) {
            PlanarGraph *plot = plotList[i];
            // 保存图表颜色配置
            QList<QColor> colors = plot->getColorList();
            QString colorStr;
            for (const QColor &color : colors) {
                if (!colorStr.isEmpty()) colorStr += ",";
                colorStr += color.name();
            }
            pConfig.setValue("Card" + QString::number(i + 1) + "_chart_colors", colorStr);

            // 保存线条宽度
            pConfig.setValue("Card" + QString::number(i + 1) + "_line_width", plot->getLineWidth());

            // 保存背景颜色
            pConfig.setValue("Card" + QString::number(i + 1) + "_background_color", plot->getBackgroundColor().name());
        }
    ...
    for (int i = 0; i < plotList.size(); i++) {
        if (plotList[i] != nullptr) {
            pConfig.setValue("Window_" + QString::number(i) + "_maximized", plotList[i]->isMaximized());
            pConfig.setValue("Window_" + QString::number(i) + "_geometry", plotList[i]->saveGeometry());
        }
    }
    pConfig.endGroup();
